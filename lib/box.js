...
function Box(obj) {
            ...
            attributes.forEach(function(attribute) {
                ...
                if (data[attribute] != box[attribute]) {
                    ...
                    tagData.forEach(function(tagDetails, i) {
                        if (tagDetails.properties.includes(attribute)) {
                            getProps(tag[i], tagProp[i]);
                        }
                    })
            ...
            elements.forEach(function(child) {
                if (!scanListenedElement.includes(child)) {
                    ...
                    if (attributes.length != 0) {
                        BoxAttributes.forEach(function(BoxAttribute) {
    ...
    function getProps(child, property) {
        ...
        if (!tag.includes(child)) {
            ...
            tagProp[index] = property;
            tagData[index] = {};
            tagData[index].mapper = child.getAttribute(property);
            tagData[index].properties = [];
            elemProps = tagData[index].properties;
